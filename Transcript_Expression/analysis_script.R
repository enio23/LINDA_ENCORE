...
foreach(ii = 1:length(koGenes)) %dopar% {
  ...
  print(paste0("Step --- ", ii, "/", length(koGenes)))
  ...
  # Download KO
  experiments <- temp_ko$V1[which(grepl(pattern = "transcript_quantifications_GRCh38.tsv.gz", x = temp_ko$V1, fixed = TRUE))]
  ...
  for(jj in 1:length(experiments)){
    if(grepl(pattern = "transcript_quantifications", x = experiments[jj])){
      ...
      if("transcript_id"%in%colnames(temp)){
  ...
  commonGenes <- intersect(x = intersect(x = ko1$transcript_id, y = ko2$transcript_id), 
                           y = intersect(x = ctrl1$transcript_id, y = ctrl2$transcript_id))
  
  if(length(commonGenes)>0){
    
    ko1 <- ko1[which(ko1$transcript_id%in%commonGenes), ]
    ko2 <- ko2[which(ko2$transcript_id%in%commonGenes), ]
    ctrl1 <- ctrl1[which(ctrl1$transcript_id%in%commonGenes), ]
    ctrl2 <- ctrl2[which(ctrl2$transcript_id%in%commonGenes), ]
    
    ko1 <- ko1[order(ko1$transcript_id), ]
    ko2 <- ko2[order(ko2$transcript_id), ]
    ctrl1 <- ctrl1[order(ctrl1$transcript_id), ]
    ctrl2 <- ctrl2[order(ctrl2$transcript_id), ]
    
    idx2rem <- which(duplicated(ko1$transcript_id)); if(length(idx2rem)>0){ko1 <- ko1[-idx2rem, ]}
    idx2rem <- which(duplicated(ko2$transcript_id)); if(length(idx2rem)>0){ko2 <- ko2[-idx2rem, ]}
    ...
    df <- as.data.frame(df)
    rownames(df) <- sapply(strsplit(x = ko1$transcript_id, split = ".", fixed = TRUE), '[', 1)
    ...
    save(ttop, file = paste0("output/hepg2/ttop_", tolower(koGenes)[ii], "_hepg2.RData"))
    
    es_data <- ttop[, c(6, 1, 5)]
    colnames(es_data) <- c("id", "effect", "significance")
    save(es_data, file = paste0("output/hepg2/es_data_", tolower(koGenes)[ii], "_hepg2.RData"))
...
foreach(ii = 1:length(koGenes)) %dopar% {
  ...
  if(length(commonGenes)>0){
    ...
    ttop=as.data.frame(topTags(res,n=nrow(df)))
    ttop$ID <- rownames(ttop)
    save(ttop, file = paste0("output/k562/ttop_", tolower(koGenes)[ii], "_k562.RData"))
    
    es_data <- ttop[, c(6, 1, 5)]
    colnames(es_data) <- c("id", "effect", "significance")
    save(es_data, file = paste0("output/k562/es_data_", tolower(koGenes)[ii], "_k562.RData"))
