...
#   
#   # if(sources=="Perturbation"){
#   # 
#   #   #
#   #   diffA <- setdiff(netA[, 1], netA[, 3])
#   #   idx <- which(diffA%in%sources==FALSE)
#   #   if(length(idx)>0){
#   # 
#   #     for(ii in 1:length(idx)){
#   # 
#   #       top <- diffA[idx[ii]]
#   #       ind <- which(netA[, 1]==top)
#   #       domains <- unique(sapply(strsplit(x = netA[ind, 4],
#   #                                         split = "=",
#   #                                         fixed = TRUE), "[", 1))
#   # 
#   #       if(length(domains)>0){
...
#   #         toBind[, 1] <- "Perturbation"
#   #         toBind[, 2] <- as.character(max(as.numeric(netA[ind, 2])))
#   #         toBind[, 3] <- top
#   #         toBind[, 4] <- paste0("Perturbation=", domains)
#   #         colnames(toBind) <- colnames(netA)
#   #         netA <- unique(rbind(netA, toBind))
#   # 
#   #       }
#   # 
#   #     }
...
#   # 
#   #   #
#   #   diffB <- setdiff(netB[, 1], netB[, 3])
#   #   idx <- which(diffB%in%sources==FALSE)
#   #   if(length(idx)>0){
#   # 
#   #     for(ii in 1:length(idx)){
#   # 
#   #       top <- diffB[idx[ii]]
#   #       ind <- which(netB[, 1]==top)
#   #       domains <- unique(sapply(strsplit(x = netB[ind, 4],
#   #                                         split = "=",
#   #                                         fixed = TRUE), "[", 1))
#   # 
#   #       if(length(domains)>0){
...
#   #         toBind <- matrix(data = , nrow = length(domains), ncol = 4)
#   #         toBind[, 1] <- "Perturbation"
#   #         toBind[, 2] <- as.character(max(as.numeric(netB[ind, 2])))
#   #         toBind[, 3] <- top
#   #         toBind[, 4] <- paste0("Perturbation=", domains)
#   #         colnames(toBind) <- colnames(netB)
#   #         netB <- unique(rbind(netB, toBind))
#   # 
#   #       }
#   # 
#   #     }
...
#         strsplit(
#           x = background.network$id_source[
#             which(bgReacS==attributes[ii, 1])], 
#           split = "_", 
#           fixed = TRUE)),
#         unlist(
#           strsplit(
#             x = background.network$id_target[
...
#   
#   net <- unique(net)
#   idx <- intersect(x = which(net[, 4]=="spliced"), 
#                    y = which(net[, 6]!="1"))
#   
#   if(length(idx)>0){
#     domains <- unique(c(net[idx, 1], net[idx, 2]))
#     domains <- domains[which(grepl(pattern = "(", x = domains, fixed = TRUE))]
#     
#     idx1 <- which(net[, 1]%in%domains)
#     idx2 <- which(net[, 2]%in%domains)
#     
#     ind <- unique(c(idx1, idx2))
#     net[idx1, 1] <- sapply(strsplit(x = net[idx1, 1], split = "(", fixed = TRUE), "[", 1)
#     net[idx2, 2] <- sapply(strsplit(x = net[idx2, 2], split = "(", fixed = TRUE), "[", 1)
#     net[ind, 4] <- "normal"
#     
#   }
#   
...
prepare_cytoscape_visualization <- function(netA = netA,
  ...
  if(length(ind)>0){
    ...
    for(ii in 1:length(uReac)){
      ...
      if(length(intersect(x = "Common", y = network[idx2, 3]))>0 ||
         length(intersect(x = c("Network_A", "Network_B"), y = network[idx2, 3]))==2){
